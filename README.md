# Middlegame_Chess
Want to play Chess but tired of memorising openings? This variant of chess loads up a starting Middlegame position deemed equal for White and Black by Stockfish (Depth = 20). 

### Definitions ###
There is no definite way to classify any position that represents the end of the Opening and the start of the Middlegame. It is possible to use the stockfish analysis of all initial moves to determine which move establishes the confirmation of an Opening/ Opening Variant, however the positional database does not keep track of move orders and only stores FEN positions. Hence a rough estimate would be a position obtained after at least 10 moves. To ensure that it will not quickly evolve into a drawn endgame, initial positions will require at least all but 2 pieces to be remaining on the board.
